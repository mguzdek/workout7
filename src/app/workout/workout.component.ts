...
export class WorkoutComponent implements OnInit, OnDestroy {
  ...
  workoutHistoryTrackerService: WorkoutHistoryTrackerService;

  constructor(private router: Router, workoutHistoryTrackerService: WorkoutHistoryTrackerService) {
      this.workoutHistoryTrackerService = workoutHistoryTrackerService;
   }
    ...
    this.exerciseTrackingInterval = window.setInterval(() => {
      if (this.exerciseRunningDuration >= this.currentExercise.duration) {
        ...
        if (this.currentExercise !== this.restExercise) {
          this.workoutHistoryTrackerService.exerciseComplete(this.workout.exercises[this.currentExerciseIndex]);
        }
  ...
  buildWorkout(): Workout {
    const workout = new Workout('7MinWorkout', '7 Minute Workout', 10, []);
